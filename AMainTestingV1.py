#!/usr/bin/env python3
"""
OKX Futures Scanner — Streamlit Dashboard
Updated: Dubai timezone, revised defaults, dark-green MACD, criteria/leverage/SL-reason columns.
"""
...
def dubai_now() -> datetime:
    """Return current time in Dubai timezone (UTC+4 / GST)."""
    return datetime.now(DUBAI_TZ)

def to_dubai(dt: datetime) -> datetime:
    """Convert any datetime to Dubai timezone."""
    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=timezone.utc)
    return dt.astimezone(DUBAI_TZ)

def fmt_dubai(iso_str: str, fmt: str = "%m/%d %H:%M") -> str:
    """Parse an ISO timestamp string and format it in Dubai time."""
    try:
...
# ─────────────────────────────────────────────────────────────────────────────
DEFAULT_CONFIG: dict = {
    "tp_pct":             1.5,   # CHANGE 1: was 2.0
    "sl_pct":             3.0,   # CHANGE 1: was 1.0
    "rsi_5m_min":         30,    # CHANGE 3: was 45
    "resistance_tol_pct": 1.5,
    "rsi_1h_min":         30,    # CHANGE 4: was 40
    "rsi_1h_max":         95,    # CHANGE 4: was 72
    "loop_minutes":       5,     # CHANGE 6: was 3
    "cooldown_minutes":   5,     # CHANGE 5b: was 30
    # ── EMA per-timeframe ────────────────────────────────────────────────────
    "use_ema_3m":         False,
    "ema_period_3m":      200,
    "use_ema_5m":         True,
...
# ─────────────────────────────────────────────────────────────────────────────
# CHANGE 9 — Max leverage tiers per coin
# ─────────────────────────────────────────────────────────────────────────────
MAX_LEVERAGE: dict = {
    # Tier 1 — 125×
    "BTCUSDT": 125,
    ...
    # Default for all others: 20× (applied in get_max_leverage)
}

def get_max_leverage(sym: str) -> int:
    """Return the maximum allowed leverage for a given symbol."""
    return MAX_LEVERAGE.get(sym, 20)

# ─────────────────────────────────────────────────────────────────────────────
# CHANGE 10 — SL reason analyzer
# ─────────────────────────────────────────────────────────────────────────────
def analyze_sl_reason(sig: dict) -> str:
    """
    Given a signal that hit SL, diagnose likely reasons using
    the criteria snapshot captured at entry time.
    """
...
def macd_bullish(closes: list, crossover_lookback: int = 12) -> bool:
    """
    CHANGE 5 — Dark-green histogram required.

    Returns True when ALL of the following hold:
      1. MACD line  > 0  (above zero — bullish territory)
      2. Signal line > 0  (signal in positive zone)
      3. Histogram  > 0  AND histogram[-1] > histogram[-2]
         (dark green: momentum ACCELERATING upward, not fading light green)
      4. A bullish crossover (MACD crossed above signal) within the last
    ...
    # Conditions 1 & 2: above zero
    if macd_line[-1] <= 0 or sig_line[-1] <= 0:
        return False

    # Condition 3: histogram positive AND increasing (dark green only)
    if histogram[-1] <= 0 or histogram[-1] <= histogram[-2]:
        return False

    # Condition 4: recent bullish crossover
...
def process(sym, cfg: dict):
    try:
        ...
        # ── F9: MACD dark-green bullish (3m, 5m & 15m) ───────────────────────
        ...
        # ── CHANGE 8: Snapshot indicator values at entry time ─────────────────
        criteria = {
            "rsi_5m":   round(rsi5,  1),
        ...
        return {
            "id":          str(uuid.uuid4())[:8],
            "timestamp":   dubai_now().isoformat(),   # CHANGE 2: Dubai time
            "symbol":      sym,
            "entry":       entry,
            "tp":          tp,
            "sl":          sl,
            ...
            "close_price": None,
            "close_time":  None,
            "max_lev":     max_lev,   # CHANGE 9
            "criteria":    criteria,  # CHANGE 8
        }
    ...
    for sig in signals:
        ...
        try:
            ...
            if tp_time is not None or sl_time is not None:
                # CHANGE 7: store close_time in Dubai time
                if tp_time is not None and (sl_time is None or tp_time <= sl_time):
                    sig.update(
                        status      = "tp_hit",
                        close_price = sig["tp"],
                        close_time  = to_dubai(
                            datetime.fromtimestamp(tp_time / 1000, tz=timezone.utc)
                        ).isoformat(),
                    )
                else:
                    sig.update(
                        ...
                        close_price = sig["sl"],
                        close_time  = to_dubai(
                            datetime.fromtimestamp(sl_time / 1000, tz=timezone.utc)
                        ).isoformat(),
                    )
        except Exception:
            pass
    ...
    while True:
        ...
        try:
            ...
            with _log_lock:
                ...
                active = {s["symbol"] for s in _b._bsc_log["signals"]
                          if s["status"] == "open"}
                skip   = cooled | active
                for sig in new_sigs:
                ...
                _b._bsc_log["health"].update(
                    ...
                    last_scan_at         = dubai_now().isoformat(),   # CHANGE 2
...
with st.sidebar:
    ...
    new_use_ema_3m = ea1.checkbox("3m EMA",
        value=bool(_snap_cfg.get("use_ema_3m", False)), key="cfg_use_ema_3m",
        help="Price must be above EMA on the 3-minute chart.")
    new_ema_period_3m = ea2.number_input("Period##3m", min_value=2, max_value=500, step=1,
        value=int(_snap_cfg.get("ema_period_3m", 200)), key="cfg_ema_period_3m",
        disabled=not new_use_ema_3m, label_visibility="collapsed")
    if new_use_ema_3m:
        ea2.caption(f"3m EMA {new_ema_period_3m}")

    eb1, eb2 = st.columns([1, 2])
    new_use_ema_5m = eb1.checkbox("5m EMA",
        value=bool(_snap_cfg.get("use_ema_5m", True)), key="cfg_use_ema_5m",
        help="Price must be above EMA on the 5-minute chart.")
    new_ema_period_5m = eb2.number_input("Period##5m", min_value=2, max_value=500, step=1,
        value=int(_snap_cfg.get("ema_period_5m", 200)), key="cfg_ema_period_5m",
        disabled=not new_use_ema_5m, label_visibility="collapsed")
    if new_use_ema_5m:
        eb2.caption(f"5m EMA {new_ema_period_5m}")

    ec1, ec2 = st.columns([1, 2])
    new_use_ema_15m = ec1.checkbox("15m EMA",
        value=bool(_snap_cfg.get("use_ema_15m", True)), key="cfg_use_ema_15m",
        help="Price must be above EMA on the 15-minute chart.")
    new_ema_period_15m = ec2.number_input("Period##15m", min_value=2, max_value=500, step=1,
        value=int(_snap_cfg.get("ema_period_15m", 200)), key="cfg_ema_period_15m",
        disabled=not new_use_ema_15m, label_visibility="collapsed")
    if new_use_ema_15m:
    ...
    new_use_macd = st.checkbox("Enable MACD filter",
        ...
        help=(
            ...
            "③ Histogram > 0 AND increasing (🟢 dark green only — not fading light green)  "
            "④ Bullish crossover within last 12 candles"
        ))
    if new_use_macd:
        st.caption("✅ MACD > 0 · Signal > 0 · Histogram 🟢↑ dark green · Crossover ↑")
    st.divider()

    # ── Parabolic SAR Filter ──────────────────────────────────────────────────
    st.markdown("**🪂 F10 — Parabolic SAR** (3m, 5m & 15m)")
    new_use_sar = st.checkbox("Enable Parabolic SAR filter",
    ...
    new_use_vol_spike = st.checkbox("Enable volume spike filter",
        value=bool(_snap_cfg.get("use_vol_spike", False)), key="cfg_use_vol_spike",
        help="Last complete 15m candle volume must be ≥ X × the average volume of the previous N candles.")
    vx1, vx2 = st.columns(2)
    new_vol_mult = vx1.number_input("Multiplier (X×)", min_value=1.0, max_value=20.0, step=0.5,
    ...
    new_vol_lookback = vx2.number_input("Lookback (N candles)", min_value=2, max_value=100, step=1,
    ...
    if new_use_vol_spike:
        st.caption(f"✅ Last 15m volume ≥ {new_vol_mult}× avg of last {new_vol_lookback} candles")
    st.divider()
...
# CHANGE 2: show Dubai time in header
...
col_h1.caption(f"Last scan: {last_scan}   |   Auto-refreshes every 30 s   |   🕐 All times: Dubai / GST (UTC+4)")
...
if filtered_sorted:
    ...
    for s in filtered_sorted:
        ...
        status_icon = {"open": "🔵 Open", "tp_hit": "✅ TP Hit",
                       "sl_hit": "❌ SL Hit"}.get(status, status)

        # CHANGE 2: entry time in Dubai
        ts_str = fmt_dubai(s.get("timestamp", ""))

        # CHANGE 7: close time in a separate column (Dubai)
        close_ts     = s.get("close_time")
        close_ts_str = fmt_dubai(close_ts) if close_ts else "—"

        # CHANGE 8: entry criteria snapshot as bullets
        crit = s.get("criteria", {})
        if crit:
            criteria_str = (
        ...
        # CHANGE 9: max leverage
        max_lev = s.get("max_lev", get_max_leverage(s.get("symbol", "")))

        # CHANGE 10: SL reason (only for sl_hit)
        ...
        rows.append({
            ...
            "SL":              s.get("sl", ""),
            "Status":          status_icon,
            "Close Time":      close_ts_str,
            "Close $":         s.get("close_price") or "—",
            "Max Lev":         f"{max_lev}×",
            "Entry Criteria":  criteria_str,
...
if signals:
    ...
    if len(signals) > 1:
        ...
        for s in signals:
            try:
                d = to_dubai(datetime.fromisoformat(
                    s["timestamp"].replace("Z", "+00:00"))).strftime("%m/%d")
                day_counts[d] += 1
            except Exception:
                pass
        if day_counts:
            ...
            fig_line.update_layout(
                title="Signals Per Day (Dubai / GST)",
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                font=dict(color="#e6edf3"),
                yaxis=dict(gridcolor="#21262d"),
                margin=dict(t=40, b=10, l=10, r=10),
